...
buildStockAnalysis(filters) {
  ...
  if (filters.accountType && filters.accountType.length > 0) {
    matchConditions.party = { $in: filters.accountType.map(id => new mongoose.Types.ObjectId(id)) };
  }

  // Initial filtering from Registry
  pipeline.push({ $match: matchConditions });

  // Join with accounts for party details first (since party is in Registry)
  pipeline.push({
    $lookup: {
      from: "accounts",
      let: { partyId: "$party" },
  ...
  pipeline.push({ $unwind: { path: "$partyDetails", preserveNullAndEmptyArrays: true } });
  ...
  pipeline.push({
    $project: {
      ...
      Account: { $ifNull: ["$partyDetails.customerName", "N/A"] }, // Fixed: using partyDetails.customerName
      Pcs: {
        $cond: {
          if: filters.showPcs,
          then: {
